...
examples.forEach(([sku, info]) => {
  console.log(`  ${sku} -> ${info.image.substring(0, 40)} (${info.matchType})`);
});
...
function extractKeywords(description, specification) {
  ...
  // 提取容量
  const capacityMatch = (description + ' ' + specification).match(/(\d+(?:\.\d+)?)\s*(ml|ul|l)/i);
  if (capacityMatch) {
  ...
  features.forEach(feature => {
    if (description.includes(feature) || specification.includes(feature)) {
